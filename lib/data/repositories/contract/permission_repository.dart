...
class PermissionRepository extends GetxController {
  ...
  ///
    ...
    try {
      ...
      return response
          .map((pm) => PermissionModel.fromJson(pm))
          .toList();
    } catch (e) {
  ...
  Future<bool> removePermission(PermissionModel permission) async {
    try {
      // first update the fields
      int? pi = permission.permissionId;
      if (pi == null) {
        debugPrint('Permission ID is null, cannot remove permission.');
        return false;
      }
      debugPrint('Removing permission with ID: $pi');
      final result = await _objectService.removePermission(
        pi,
      );

      if (result['success'] == false) {
        debugPrint(
          'Error removing permission : ${result['message']}',
        );
        return false;
      }
      ...
      debugPrint('Error removing permission: $e');
      return false;
    }
  }
  ...
  Future<bool> createPermission(int userId,int objectId,int roleId) async {
    try {
      ...
      debugPrint('[Permission Repository] Creating Permission: $userId $objectId $roleId');
      // first update the fields
      ...
      if (result['success'] == false) {
        debugPrint('Error creating permission : ${result['message']}');
        return false;
      }
      ...
      debugPrint('Error submitting a new permission: $e');
      return false;
    }
  }
